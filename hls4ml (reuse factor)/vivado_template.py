...
film_function_template = 'nnet::film<{input1_t}, {input2_t}, {output_t}, {config}>({input1}, {input2}, {output});'
mask_track_function_template = 'nnet::mask_track<{input_t}, {output_t}, {config}>({input}, {output});'
sum1d_function_template = 'nnet::sum1d<{input_t}, {output_t}, {config}>({input}, {output});'
slice_tensor1d_function_template = 'nnet::slice_tensor1d<{input_t}, {output_t}, {config}>({input}, {output});'
...
class VivadoBackend(Backend):
    def __init__(self, name='Vivado'):
        ...
        self.register_templates('FiLM'                   , film_function_template,        film_config_template, deepcalo_include_list)
        self.register_templates('Mask_track' , mask_track_function_template, mask_track_config_template, deepcalo_include_list)
        self.register_templates('Sum1D' , sum1d_function_template, sum1d_config_template, deepcalo_include_list)
        self.register_templates('Slice_tensor1D' , slice_tensor1d_function_template, slice_tensor1d_config_template, deepcalo_include_list)
    ...
    def get_valid_reuse_factors(self, layer):
        ...
        # add get precision 2022/03
        input_presicion_width = 0
        ...
        if 'Dense' in layer.__class__.__name__:
            ...
            # add get precision 2022/03
            input_presicion_width = layer.get_input_variable().type.precision.width
            #tmp = input('input_presicion_width')
            print('Dense input width: ',input_presicion_width)
            #print(type(input_presicion_width))
            ...
            # add get precision 2022/03
            input_presicion_width = layer.get_input_variable().type.precision.width
            print('Conv1D input width: ',input_presicion_width)
            ...
            # add get precision 2022/03
            input_presicion_width = layer.get_input_variable().type.precision.width
            print('Conv2D input width: ',input_presicion_width)
            

        max_rf = n_in * n_out
        valid_reuse_factors = []
        for rf in range(1, max_rf + 1):
            _assert = self._check_conditions(n_in, n_out, rf, input_presicion_width)
            if _assert:
    ...
    # add get precision 2022/03
    def _check_conditions(self, n_in, n_out, rf, input_presicion_width):
        ...
        # 2022/03
        # check whether (input_presicion_width * block_factor)  exceeds vivado 65535 bitwidth, but let max bitwidth be 8192 for safe
        _rf = min(n_in * n_out, rf)
        block_factor = int(math.ceil((n_in * n_out) / float(_rf)))
        
        _assert = _assert and (input_presicion_width * block_factor) <= 8192
